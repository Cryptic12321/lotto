...
function formatTime(timestamp: number | null) {
  ...
  return new Intl.DateTimeFormat(undefined, { year: 'numeric', month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit', timeZoneName: 'short' }).format(timestamp * 1000)
}
...
export function LiveTransactions() {
  ...
  return <section className="transactions-section" aria-labelledby="transactions-title">
    <div className="section-heading"><div><span className="eyebrow">TRANSPARENCY</span><h2 id="transactions-title">LIVE TRANSACTIONS</h2><span className="mainnet-activity-label">REAL-TIME ACTIVITY · SOLANA MAINNET</span></div><span className="live-pill"><i aria-hidden="true" />LIVE</span></div>
    {loading ? <div className="transaction-state"><RefreshCw className="spin" aria-hidden="true" /> LOADING MAINNET ACTIVITY</div> : error ? <div className="transaction-state">LIVE TRANSACTION FEED TEMPORARILY UNAVAILABLE</div> : rows.length === 0 ? <div className="transaction-state">NO TRANSACTIONS FOUND</div> : <div className="transaction-list">{rows.map((row) => <a className="transaction-row" href={row.solanaUrl} target="_blank" rel="noreferrer" key={row.signature}><span className={`transaction-type transaction-type-${row.type.toLowerCase()}`}>{row.type}</span><span className="transaction-signature">{row.signature.slice(0, 8)}…{row.signature.slice(-8)}</span><span className="transaction-time">{formatTime(row.timestamp)}</span><ArrowUpRight aria-hidden="true" /></a>)}</div>}
  </section>
}
