...
function DrawCard({ poolBalance, poolBalanceError }: { poolBalance: number | null; poolBalanceError: boolean }) {
  ...
  return <article className="draw-card draw-card-red"><div className="draw-card-header"><span className="draw-title"><span className="draw-symbol">✦</span>DAILY LOTTO</span><span className="demo-tag">{poolStatus}</span></div><span className="eyebrow">CURRENT POT</span><strong className={`pot-value ${poolBalance !== null ? '' : 'pot-status'}`}>{poolStatus}</strong><div className="timer-wrap"><span className="eyebrow">DRAWING IN</span><strong className={`countdown ${draw.drawing ? 'drawing' : ''}`}>{draw.drawing ? 'DRAWING...' : formatTimer(draw.remaining)}</strong><span className="timer-label">NEXT DRAW &nbsp; EVERY DAY AT 6:00 PM ET</span></div><div className="holder-line"><strong>10,000</strong><span>LOTTO MINIMUM</span></div><p>One verified eligible holder wins the daily pot.</p></article>
}
