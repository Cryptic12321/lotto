...
import { LottoPoolSplit } from '@/components/lotto-pool-split'
...
function useHourlyDraw() { return useDrawCountdown(nextHourlyDraw) }
function useDailyDraw() { return useDrawCountdown(nextDailyDraw) }
...
function DailyDrawCard({ poolBalance, poolBalanceError, solPrice, solPriceError }: { poolBalance: number | null; poolBalanceError: boolean; solPrice: number | null; solPriceError: boolean }) { const draw = useDailyDraw(); const value = poolBalance !== null && solPrice !== null ? poolBalance * solPrice : null; return <article className="draw-card draw-card-daily"><div className="draw-card-header"><span className="draw-title"><span className="draw-symbol">✦</span>DAILY LOTTO</span><span className="demo-tag">FEATURED DRAW</span></div><span className="eyebrow">CURRENT DAILY POOL</span><strong className="pot-value">{value !== null ? `$${value.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : poolBalanceError || solPriceError ? 'POOL VALUE UNAVAILABLE' : 'LOADING POOL VALUE'}</strong><div className="timer-wrap"><span className="eyebrow">DRAWING IN</span><strong className={`countdown ${draw.drawing ? 'drawing' : ''}`}>{draw.drawing ? 'DRAW TIME' : formatTimer(draw.remaining)}</strong><span className="timer-label">NEXT DRAW · EVERY DAY AT 6:00 PM ET</span></div><p>One verified eligible holder wins the daily pot.</p></article> }
...
function DrawCard({ poolBalance, poolBalanceError, solPrice, solPriceError }: { poolBalance: number | null; poolBalanceError: boolean; solPrice: number | null; solPriceError: boolean }) {
  ...
  const hourlyWinnerWallet = '59RHVo2EFewE7E6pYhZYEQT3UXVWfoEcQdfPZKp2ciKX'
  const [hourlyWinnerCopied, setHourlyWinnerCopied] = useState(false)
  ...
  return <article className="draw-card draw-card-red"><div className="draw-card-header"><span className="draw-title"><span className="draw-symbol">✦</span>HOURLY LOTTO</span><span className="demo-tag">{poolStatus}</span></div><span className="eyebrow">CURRENT POOL / REWARDS WALLET VALUE</span>{usdValue !== null ? <strong className="pot-value">${usdValue.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</strong> : <strong className="pot-value pot-status">{solPriceError ? 'USD VALUE TEMPORARILY UNAVAILABLE' : poolStatus}</strong>}{poolBalance !== null && <span className="pool-sol-secondary">{poolBalance.toFixed(2)} SOL<br /><small>Live value · updates automatically</small></span>}<div className="timer-wrap"><span className="eyebrow">DRAWING IN</span><strong className={`countdown ${draw.drawing ? 'drawing' : ''}`}>{draw.drawing ? 'DRAW TIME' : formatTimer(draw.remaining)}</strong><span className="timer-label">NEXT DRAW &nbsp; EVERY HOUR ON THE HOUR · LOCAL TIME</span></div><div className="holder-line"><strong>10,000</strong><span>LOTTO MINIMUM</span></div><p>One verified eligible holder wins the hourly pot.</p><div className="hourly-winner-card"><strong>🎉 CONGRATULATIONS TO OUR HOURLY LOTTO WINNER!</strong><span>{hourlyWinnerWallet.slice(0, 9)}...{hourlyWinnerWallet.slice(-7)}</span><div className="winner-actions"><button type="button" onClick={async () => { await navigator.clipboard.writeText(hourlyWinnerWallet); setHourlyWinnerCopied(true); window.setTimeout(() => setHourlyWinnerCopied(false), 1600) }}>{hourlyWinnerCopied ? 'COPIED' : 'COPY WALLET'}</button><a href={`https://solscan.io/account/${hourlyWinnerWallet}`} target="_blank" rel="noreferrer">VIEW ON SOLSCAN <ExternalLink /></a></div></div></article>
}
