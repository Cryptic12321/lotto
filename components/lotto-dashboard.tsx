...
import { LOTTO_MINT, LOTTO_POOL_WALLET, POOL_CONNECTED, POOL_WALLET_CONFIGURED, POOL_WALLET_VALID, SOLANA_NETWORK, TOKEN_CONNECTED, explorerAddress, explorerToken, getPoolBalance, getSplBalance, shortAddress } from '@/lib/solana'
...
function DrawCard({ poolBalance, poolBalanceError }: { poolBalance: number | null; poolBalanceError: boolean }) {
  const draw = useDailyDraw()
  const connected = POOL_CONNECTED && poolBalance !== null
  const poolStatus = !POOL_WALLET_CONFIGURED || !POOL_WALLET_VALID ? 'POOL WALLET NOT CONFIGURED' : poolBalanceError ? 'POOL BALANCE UNAVAILABLE' : connected ? `${poolBalance.toFixed(2)} SOL` : 'LOADING POOL BALANCE'
  return <article className="draw-card draw-card-red"><div className="draw-card-header"><span className="draw-title"><span className="draw-symbol">✦</span>DAILY LOTTO</span><span className="demo-tag">{poolStatus}</span></div><span className="eyebrow">CURRENT POT</span><strong className={`pot-value ${connected ? '' : 'pot-status'}`}>{poolStatus}</strong><div className="timer-wrap"><span className="eyebrow">DRAWING IN</span><strong className={`countdown ${draw.drawing ? 'drawing' : ''}`}>{draw.drawing ? 'DRAWING...' : formatTimer(draw.remaining)}</strong><span className="timer-label">NEXT DRAW &nbsp; EVERY DAY AT 6:00 PM ET</span></div><div className="holder-line"><strong>10,000</strong><span>LOTTO MINIMUM</span></div><p>One verified eligible holder wins the daily pot.</p></article>
}
...
export function LottoDashboard() {
  ...
  useEffect(() => { if (!POOL_CONNECTED) return; getPoolBalance(LOTTO_POOL_WALLET).then((value) => { setPoolBalance(value); setPoolBalanceError(false) }).catch(() => { setPoolBalance(null); setPoolBalanceError(true) }) }, [])
